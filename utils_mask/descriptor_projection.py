...
features_source = "/home/leo/Desktop/Leo/SEQUOIA/Modular-GS/data/figurines_mask/"+id+"/features"
camera_sources = "/home/leo/Desktop/Leo/SEQUOIA/Modular-GS/data/figurines_mask/"+id+"/cameras"

# features = torch.load(features_source, weights_only= True)
# cameras = torch.load(camera_sources, weights_only= True)
...
# features = features.view(72, 40, 40, 384)
# features = features.permute(0, 3, 1, 2)

# # Upsample to [72, 384, 1120, 1120]
# features = F.interpolate(features, size=(1120, 1120), mode='bilinear', align_corners=False)

# # Permute back to [72, 1120, 1120, 384]
...
n_azim = 12
radius = 2.0 
#center = compute_centroid(PlyData.read(gs_path))
...
for camera in tqdm(cameras, desc="Descriptors projection...", total=len(cameras)):
    ...
    W = H = resolution
    #random_pixels = torch.rand((W, H, 384), device="cuda")
...
if isinstance(desc_tensor, torch.Tensor):
    ...
    # KMeans clustering
    # n_clusters = 5  # You can change this value as needed
    # clusters = SpectralClustering(n_clusters=n_clusters, assign_labels='kmeans', affinity='nearest_neighbors')
    ...
    # ax = fig.add_subplot(111, projection='3d')
    # scatter = ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2], c=norm_pca, cmap='viridis', s=5, alpha=0.8)
    # ax.set_title('Gaussians in 3D colored by PCA(1) of descriptors')
    # fig.colorbar(scatter, ax=ax, label='PCA(1)')
    # plt.tight_layout()
    # plt.show()
