...
if __name__ == "__main__":
    ...
    parser.add_argument("--scene", default="scene0347_00", type=str, help="scene to process (e.g., 'scene0347_00').")
    ...
    # cameras = cameras[::20] #!REMOVE IF ALREADY SUBSAMPLED
...
#SAM model
    ...
    mask_folder =  os.path.join("./data", SCENE, "masks")

    # Load GS reconstruction
    ply_path = os.path.join("./output", SCENE, "raw", INSTANCE_TEST, "point_cloud/iteration_1000/point_cloud.ply")
    ...
    INSTANCE_TEST = args.instance_test
    print(f"Processing Instance ID: {INSTANCE_TEST}")
    INSTANCE_PATH = os.path.join("./data", SCENE, "masks", INSTANCE_TEST)

    os.makedirs(os.path.join(f"ref_{SCENE}",INSTANCE_TEST) , exist_ok=True)
    iter = 0
    ...
    for cam in tqdm(cameras):
        ...
        if(VERBOSE): print(f"computing: {img_name}")
        ...
        try:
            ...
            mask_img = IMG.open(os.path.join(mask_folder,INSTANCE_TEST,"masks", mask_name)).convert("RGBA")#.convert("L")
            alpha = np.array(mask_img)[:, :, 3]

            # Mask is True where alpha > 0 (visible areas)
            mask = alpha > 0
        except:
            path = os.path.join(mask_folder, INSTANCE_TEST,"masks", mask_name)
            if(VERBOSE): print(f"mask{path} is none!")
            mask = None
        ...
        try:
            ...
            if(VERBOSE): print(f"Warning: dilation failed ({e}), using original alpha.")
            pass
        ...
        os.makedirs(os.path.join(INSTANCE_PATH, "masks_extra"), exist_ok=True)
        mask_extra_path = os.path.join(INSTANCE_PATH, "masks_extra", save_name)
        if(VERBOSE): print(f"SAVE PATH: {mask_extra_path}")
