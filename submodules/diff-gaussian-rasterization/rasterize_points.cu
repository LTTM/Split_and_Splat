	...
	const bool debug)
{
  ...
  //Define pixels shape
  torch::Tensor pixels = torch::zeros({W*H, 3}, means3D.options());
  torch::Tensor pixel_index_tensor = torch::zeros({1}, int_opts);
  int* pixel_index = pixel_index_tensor.data_ptr<int>();
  ...
  return std::make_tuple(rendered, pixel_index_tensor, out_color, radii, pixels, geomBuffer, binningBuffer, imgBuffer, out_invdepth);
}
