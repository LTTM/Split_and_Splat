...
from scene.colmap_loader import read_extrinsics_text, read_intrinsics_text, qvec2rotmat, \
    ...
    read_extrinsic_JSON, read_intrisics_JSON, filterPLY, filterPLY_blender
...
def fetchPly(path):
    ...
    if all(c in vertices.data.dtype.names for c in ('red', 'green', 'blue')):
        ...
        colors = np.zeros_like(positions)  # same shape, 3 columns
...
def readColmapSceneInfo(path, images, depths, eval, train_test_exp, llffhold=8 ,instance = True):
    try:
    ...
    if not os.path.exists(ply_path):
        ply_scene_path = "sparse/0/points3D.ply"
        bin_scene_path = os.path.join(path, "sparse/0/points3D.bin")
        txt_scenepath = os.path.join(path, "sparse/0/points3D.txt")
        if not os.path.exists(ply_scene_path):
            print("Converting point3d.bin to .ply, will happen only the first time you open the scene.")
            try:
                xyz, rgb, _ = read_points3D_binary(bin_scene_path)
            except:
                xyz, rgb, _ = read_points3D_text(txt_scenepath)
            ply_scene_path = os.path.join(path, "sparse/0/points3D.ply")
            storePly(ply_scene_path, xyz, rgb)    
        if(instance):
            print("filtering PLY...")
...
def readCamerasFromTransforms(path, transformsfile, depths, white_background, extension=".png"):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames), desc="load images"):
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX,
                            ...
                            image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1], is_test=False))
            
    return cam_infos

def readNerfSyntheticInfo(path, white_background, depths, eval, extension=".png", instance = True):

    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", depths, white_background, extension)
    print("Reading Test Transforms")
    if os.path.exists(os.path.join(path, "transforms_test.json")):
        test_cam_infos = readCamerasFromTransforms(path, "transforms_test.json", depths, white_background, extension)
    else:
    ...
    if not os.path.exists(ply_path):
        ...
        for cam in train_cam_infos:
            cam_extrinsics[cam.image_name] = {
                "R": cam.R.transpose(),   # (3,3)
                "T": cam.T    # (3,)
            }
        ...
        ply_scene_path = os.path.join(parent_path, "points3d.ply")
        
        if(instance):
            print("filtering PLY...")
            filterPLY_blender(ply_scene_path, path, cam_intrinsics, cam_extrinsics)
