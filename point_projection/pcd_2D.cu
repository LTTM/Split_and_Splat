...
void pcd_to_zbuffer_kernel(
   ...
   // ZBufEntry* z_buffer   // H*W array
    ...
    // 3. Verify falling in the image or not
    if (i < 0 || i >= W || j < 0 || j >= H)
        return;
...
)
{
    ...
    pcd_to_zbuffer_kernel<<<blocks, threads>>>(
        d_points, N, d_depth, H, W, d_extr, d_intr, depth_thresh, d_points_2D, d_computed_depth
    );
