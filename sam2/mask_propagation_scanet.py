...
def update_label_points(labels, pointlabels, GT, pcd = None):
    ...
    # if pcd is not None:
    #     for l in labels:
    #         points_labels = list(labels[l])
    #         if len(points_labels) == 0:
    #             continue

            
    #         points =  [pcd[id] for id in points_labels]
    #         db = DBSCAN(eps=0.5, min_samples=10)
    #         cluster_ids = db.fit_predict(points)
    #         valid_clusters = [cid for cid in set(cluster_ids) if cid != -1]

    #         if not valid_clusters:
    #             labels[l] = []  # all noise
    #             continue

    #         largest_cluster = max(valid_clusters, key=lambda cid: np.sum(cluster_ids == cid))
    #         largest_ids = np.array(points_labels)[cluster_ids == largest_cluster].tolist()

    #         if(len(largest_ids)<10):
    #             continue

    #         labels[l] = set(largest_ids)

    #         for p in list(pointlabels.keys()):
    #             if l in pointlabels[p] and p not in labels[l]:
    #                 pointlabels[p].remove(l)
...
def get_label(labels):
    ...
    for i, label in enumerate(labels):
        # base weight
        #weight = 0.75 if label in init_labels else 0.75
        weight = 1
        
        # bonus for first element
        if i == 0:
            weight += 0.25
    ...
    # get label with max total weight
    # majority_label = max(counter, key=counter.get)
    # return majority_label, counter

    total_weight = sum(counter.values())
    probabilities = [(label, weight / total_weight) for label, weight in counter.items()]
    probabilities.sort(key=lambda x: x[1], reverse=True)  # sort by probability desc
...
if __name__ == "__main__":
    ...
    ply_path = os.path.join("./data", SCENE,"scene0347_00_vh_clean.ply")
